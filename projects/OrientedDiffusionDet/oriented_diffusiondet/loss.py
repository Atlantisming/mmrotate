...
from .head import rbox2mpobox, mpobox2rbox
...
class DiffusionDetCriterion(nn.Module):
    ...
    def forward(self, outputs, batch_gt_instances, batch_img_metas):
        ...
        # print(mdp_boxes.shape)

        # Compute all the requested losses
        loss_cls = self.loss_classification(outputs, batch_gt_instances,
                                            batch_indices)
        loss_bbox = self.loss_boxes(outputs, batch_gt_instances,
                                               batch_indices)
        ...
        if self.deep_supervision:
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                loss_cls = self.loss_classification(aux_outputs,
                                                    batch_gt_instances,
                                                    batch_indices)
                loss_bbox = self.loss_boxes(aux_outputs,
                                                       batch_gt_instances,
                                                       batch_indices)
                tmp_losses = dict(
                    loss_cls=loss_cls,
    ...
    def loss_boxes(self, outputs, batch_gt_instances, indices):
        ...
        target_bboxes_norm_list = [
            gt.norm_bboxes_cxcywhab[J]
            for gt, (_, J) in zip(batch_gt_instances, indices)
        ]
        target_bboxes_list = [
            gt.bboxes.tensor[J] for gt, (_, J) in zip(batch_gt_instances, indices)
        ]
        ...
        for idx in range(len(batch_gt_instances)):
            ...
            pred_bboxes = mpobox2rbox(pred_boxes[idx, indices[idx][0]].unsqueeze(0)).squeeze(0)
            ...
            #                              image_size)
            select_pred_boxes = pred_boxes[idx, indices[idx][0]]
            pred_norm_boxes = torch.cat([select_pred_boxes[:, :4] / image_size, select_pred_boxes[:, 4:]], 1)
            pred_bboxes_norm_list.append(pred_norm_boxes)
        ...
        if len(pred_boxes_cat) > 0:
            ...
            loss_bbox = self.loss_bbox(
                pred_boxes_norm_cat,
                torch.cat([
                    target_bboxes_norm_cat[:, :4], target_bboxes_norm_cat[:, 4:]], 1)) / num_instances
...
class DiffusionDetMatcher(nn.Module):
    ...
    def forward(self, outputs, batch_gt_instances, batch_img_metas):
        ...
        pred_rboxes = mpobox2rbox(pred_bboxes)
        ...
        for i in range(batch_size):
            ...
            pred_instances.rboxes = pred_rboxes[i, ...]
    ...
    def single_assigner(self, pred_instances, gt_instances, img_meta):
        with torch.no_grad():
            ...
            pred_rboxes = pred_instances.rboxes
            ...
            pairwise_ious = self.iou_calculator(pred_rboxes, gt_bboxes)
