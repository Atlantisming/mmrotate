...
from mmdet.structures.bbox import (BaseBoxes, get_box_tensor, bbox_cxcywh_to_xyxy,
                                   bbox2roi, get_box_wh, bbox_xyxy_to_cxcywh,
                                   scale_boxes)
...
class DynamicDiffusionDetHead(nn.Module):
    ...
    def prepare_training_targets(self, batch_data_samples):
        ...
        for data_sample in batch_data_samples:
            ...
            # gt_bboxes 结构 rbox 为 cxcywha
            gt_bboxes = get_box_tensor(gt_instances.bboxes.regularize_boxes(self.angle_version))
            # gt_bboxes = get_box_tensor(gt_instances.bboxes)
            h, w = img_meta['img_shape']
            image_size = gt_bboxes.new_tensor([w, h, w, h, math.pi * 0.5])
            # image_size = gt_bboxes.new_tensor([w, h, w, h, 1])

            norm_gt_bboxes_cxcywha = gt_bboxes / image_size

            pred_instances = self.prepare_diffusion(norm_gt_bboxes_cxcywha,
                                                    image_size)

            gt_instances.set_metainfo(dict(image_size=image_size))
            gt_instances.norm_bboxes_cxcywha = norm_gt_bboxes_cxcywha
    ...
    def prepare_diffusion(self, gt_boxes, image_size):
        # gt_boxes -> (cx, cy, w, h, a)
        # print('gt_boxes', gt_boxes.shape, gt_boxes)
        device = gt_boxes.device
        time = torch.randint(
            0, self.timesteps, (1,), dtype=torch.long, device=device)
        # noise = eps
        noise = torch.randn(self.num_proposals, 5, device=device)

        num_gt = gt_boxes.shape[0]
        if num_gt < self.num_proposals:
            # 3 * sigma = 1/2 --> sigma: 1/6
            box_placeholder = torch.randn(
                self.num_proposals - num_gt, 5, device=device) / 9. + 0.5
            box_placeholder[:, 2:4] = torch.clip(
                box_placeholder[:, 2:4], min=1e-4)
    ...
    def predict_by_feat(self,
        ...
        if self.use_ensemble and self.sampling_timesteps > 1:
            ...
            for idx in range(batch_size):
                ...
                # print('ensemble_score_per_img', ensemble_score_per_img)
    ...
    def do_results_post_process(results_list, cfg, batch_img_metas=None):
        ...
        for results, img_meta in zip(results_list, batch_img_metas):
            ...
            results.bboxes = torch.cat([bbox_cxcywh_to_xyxy(scale_boxes(results.bboxes[:, :4], scale_factor)), results.bboxes[:, 4:]], dim=1)
            # results.bboxes = scale_boxes(results.bboxes, scale_factor)
            # clip w, h
            # print('results.bboxes', results.bboxes)
            h, w = img_meta['ori_shape']
            results.bboxes[:, 0::2] = results.bboxes[:, 0::2].clamp(
                min=0, max=w)
            results.bboxes[:, 1:4:2] = results.bboxes[:, 1:4:2].clamp(
                min=0, max=h)
            # results.bboxes[:, 4] = results.bboxes[:, 4].clamp(
            #     min=-math.pi * 0.5, max=math.pi * 0.5)
            results.bboxes[:, 4] = norm_angle(results.bboxes[:, 4], 'le90')
            ...
            results.bboxes = torch.cat([bbox_xyxy_to_cxcywh(results.bboxes[:, :4]), results.bboxes[:, 4:]], dim=1)
    ...
    def prepare_testing_targets(self, batch_img_metas, device):
        ...
        for img_meta in batch_img_metas:
            h, w = img_meta['img_shape']
            # TODO img_size math.pi
            image_size = torch.tensor([w, h, w, h, math.pi * 0.5],
                                      dtype=torch.float32,
                                      device=device)
            noise_bboxes_raw = torch.randn((self.num_proposals, 5),
                                           device=device)
            noise_bboxes = torch.clamp(
            ...
            # noise_bboxes = torch.cat([noise_bboxes_xy, noise_bboxes[:, 4:]], dim=1)
...
class SingleDiffusionDetHead(nn.Module):
    ...
    def __init__(
        ...
        self.bboxes_delta = nn.Linear(feat_channels, 5)
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        # print('bboxes_deltas', bboxes_deltas.shape, bboxes_deltas)
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        # da = torch.clamp(da, min=-math.pi / 2, max=math.pi / 2)

        # pred_ctr_x = dx * widths[:, None] + ctr_x[:, None]
        # pred_ctr_y = dy * heights[:, None] + ctr_y[:, None]
        pred_ctr_x = dx * widths[:, None] * torch.cos(alpha) \
                     - dy * heights[:, None] * torch.sin(alpha) + ctr_x[:, None]
        pred_ctr_y = dx * widths[:, None] * torch.sin(alpha) \
                     + dy * heights[:, None] * torch.cos(alpha) + ctr_y[:, None]
        pred_w = torch.exp(dw) * widths[:, None]
        pred_h = torch.exp(dh) * heights[:, None]
        gt = norm_angle(alpha + da, self.angle_version)

        pred_boxes = torch.zeros_like(deltas)
        if self.edge_swap:
            w_regular = torch.where(pred_w > pred_h, pred_w, pred_h)
            h_regular = torch.where(pred_w > pred_h, pred_h, pred_w)
            theta_regular = torch.where(pred_w > pred_h, gt, gt + math.pi / 2)
            theta_regular = norm_angle(theta_regular, self.angle_version)
            pred_boxes[:, 0::5] = pred_ctr_x
            pred_boxes[:, 1::5] = pred_ctr_y
            pred_boxes[:, 2::5] = w_regular
            pred_boxes[:, 3::5] = h_regular
            pred_boxes[:, 4::5] = theta_regular
        else:
...
class DynamicConv(nn.Module):
    ...
    def forward(self, pro_features: Tensor, roi_features: Tensor) -> Tensor:
        ...
        Returns:
        """
        # print('pro_features', pro_features.shape, pro_features)
        # print('roi_features', roi_features.shape, roi_features)
