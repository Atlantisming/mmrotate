...
class DiffusionDetCriterion(nn.Module):
    ...
    def __init__(
            ...
            loss_riou=dict(type='RotatedIoULoss', reduction='sum', loss_weight=2.0),
    ):
        ...
        self.loss_riou = MODELS.build(loss_riou)
    ...
    def forward(self, outputs, batch_gt_instances, batch_img_metas):
        ...
        loss_cls = self.loss_classification(outputs, batch_gt_instances,
                                            batch_indices)
        loss_bbox, loss_riou = self.loss_boxes(outputs, batch_gt_instances,
                                               batch_indices)

        losses = dict(
            loss_cls=loss_cls, loss_bbox=loss_bbox, loss_riou=loss_riou)
        ...
        if self.deep_supervision:
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                loss_cls = self.loss_classification(aux_outputs,
                                                    batch_gt_instances,
                                                    batch_indices)
                loss_bbox, loss_riou = self.loss_boxes(aux_outputs,
                                                       batch_gt_instances,
                                                       batch_indices)
                tmp_losses = dict(
                    loss_cls=loss_cls,
    ...
    def loss_boxes(self, outputs, batch_gt_instances, indices):
        ...
        target_bboxes_norm_list = [
            gt.norm_bboxes_cxcywha[J]
            for gt, (_, J) in zip(batch_gt_instances, indices)
        ]
        target_bboxes_list = [
            get_box_tensor(gt.bboxes[J]) for gt, (_, J) in zip(batch_gt_instances, indices)
        ]
        ...
        # print('pred_boxes_norm_cat', pred_boxes_norm_cat)
        # print('target_bboxes_cat', target_bboxes_cat)
        # print('target_bboxes_norm_cat', target_bboxes_norm_cat)
        # print('target_bboxes_norm_cat', torch.cat([
        #             bbox_cxcywh_to_xyxy(target_bboxes_norm_cat[:, :4]), target_bboxes_norm_cat[:, 4:]], 1))
        # print('pred_boxes_cat', torch.cat([
        #                                    bbox_xyxy_to_cxcywh(pred_boxes_cat[:, :4]),
        #                                    pred_boxes_cat[:, 4:]], 1))
        # print('target_bboxes_cat', target_bboxes_cat)
        ...
        if len(pred_boxes_cat) > 0:
            ...
            #     torch.cat([
            #         bbox_cxcywh_to_xyxy(pred_boxes_norm_cat[:, :4]),
            #         pred_boxes_norm_cat[:, 4:]], 1),
            #     torch.cat([
            #         bbox_cxcywh_to_xyxy(target_bboxes_norm_cat[:, :4]), target_bboxes_norm_cat[:, 4:]], 1)) / num_instances
            loss_bbox = self.loss_bbox(
                pred_boxes_norm_cat,
                target_bboxes_norm_cat) / num_instances
            # TODO 这里写错了为什么 riou 还不是，还会变？
            # loss_riou = self.loss_riou(target_bboxes_cat,
            #                            target_bboxes_cat) / num_instances
            loss_riou = self.loss_riou(pred_boxes_cat,
                                       target_bboxes_cat) / num_instances
        else:
            loss_bbox = pred_boxes.sum() * 0
            loss_riou = pred_boxes.sum() * 0

        return loss_bbox, loss_riou
...
class DiffusionDetMatcher(nn.Module):
    ...
    def __init__(self,
        ...
        for _match_cost in match_costs:
            # 世纪大BUG 沃特么！
            if _match_cost.get('type') == 'mmdet.FocalLossCost':
    ...
    def single_assigner(self, pred_instances, gt_instances, img_meta):
        ...
        # exit()
        with torch.no_grad():
            gt_bboxes = gt_instances.bboxes             # cx, cy, w, h, a
            pred_bboxes = pred_instances.bboxes         # cx, cy, w, h, a
            ...
            for match_cost in self.match_costs:
                cost = match_cost(
                    ...
                    img_meta=img_meta)
                # print(match_cost, cost)
                cost_list.append(cost)

            # print('pred_bboxes', pred_bboxes.shape, pred_bboxes)
            # print('gt_bboxes', gt_bboxes.shape, gt_bboxes)

            pairwise_ious = self.iou_calculator(pred_bboxes, gt_bboxes)
            # print('pairwise_ious', pairwise_ious.shape, pairwise_ious)
    ...
    def get_in_gt_and_in_center_info(
        ...
        gt_xy_per_image = gt_bboxes.tensor[:, 0:2]
        gt_wh_per_image = gt_bboxes.tensor[:, 2:4]
        gt_angles_per_image = gt_bboxes.tensor[:, 4]

        gt_qboxes = rbox2qbox(gt_bboxes.tensor)
        # print('gt_qboxes', gt_qboxes)

        gt_qboxes_x = gt_qboxes[:, 0::2]
        gt_qboxes_y = gt_qboxes[:, 1::2]
        # print('gt_qboxes_x', gt_qboxes_x)
        # print('gt_qboxes_y', gt_qboxes_y)
        min_gt_x, _ = torch.min(gt_qboxes_x, 1)
        min_gt_y, _ = torch.min(gt_qboxes_y, 1)
        max_gt_x, _ = torch.max(gt_qboxes_x, 1)
        max_gt_y, _ = torch.max(gt_qboxes_y, 1)
        # print('min_gt_x', min_gt_x.unsqueeze(0).shape)
        ...
        # print('pred_bboxes_center_x', pred_bboxes_center_x.shape)
        # print('pred_bboxes_center_y', pred_bboxes_center_y.shape)

        # whether the center of each anchor is inside a gt box
        b_l = pred_bboxes_center_x > min_gt_x.unsqueeze(0)
        b_r = pred_bboxes_center_x < max_gt_x.unsqueeze(0)
        b_t = pred_bboxes_center_y > min_gt_y.unsqueeze(0)
        b_b = pred_bboxes_center_y < max_gt_y.unsqueeze(0)
        # (b_l.long()+b_r.long()+b_t.long()+b_b.long())==4 [300,num_gt] ,
        is_in_boxes = ((b_l.long() + b_r.long() + b_t.long() +
                        b_b.long()) == 4)
        # print('b_l', b_l)
        # print('is_in_boxes', is_in_boxes)
        # 判断所有 anchor 是否在gt_box里面
        is_in_boxes_all = is_in_boxes.sum(1) > 0  # [num_query]
        # print('is_in_boxes_all', is_in_boxes_all)
        ...
        b_l = pred_bboxes_center_x > (
            gt_bboxes.tensor[:, 0] -                                               # gt_bboxes.center_x
            (center_radius *
             (max_gt_x - min_gt_x))).unsqueeze(0)     # gt_bboxes.x2-x1
        b_r = pred_bboxes_center_x < (
            gt_bboxes.tensor[:, 0] +
            (center_radius *
             (max_gt_x - min_gt_x))).unsqueeze(0)
        b_t = pred_bboxes_center_y > (
            gt_bboxes.tensor[:, 1] -
            (center_radius *
             (max_gt_y - min_gt_y))).unsqueeze(0)
        ...
        is_in_centers = ((b_l.long() + b_r.long() + b_t.long() +
                          b_b.long()) == 4)
        # 判断所有 centers 是否在2.5倍大小的gt_box里面??
    ...
    def dynamic_k_matching(self, cost: Tensor, pairwise_ious: Tensor,
        ...
        targets."""
        # print('pairwise_ious', pairwise_ious.shape)
        matching_matrix = torch.zeros_like(cost)
        # select candidate topk ious for dynamic-k calculation
        candidate_topk = min(self.candidate_topk, pairwise_ious.size(0))
        # print('candidate_topk', candidate_topk)
        topk_ious, _ = torch.topk(pairwise_ious, candidate_topk, dim=0)
        # print('topk_ious', topk_ious.shape, topk_ious)
        ...
        for gt_idx in range(num_gt):
            # print('cost[:, gt_idx]', cost[:, gt_idx].shape)
            # print('dynamic_ks[gt_idx]', dynamic_ks[gt_idx])
